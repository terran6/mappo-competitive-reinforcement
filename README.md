...
The approach utilizes the power of Actor-Critic networks in order to optimize a policy and achieve expertise in the predefined domain.  Starting the process knowing nothing about what actions would lead to a positive or negative result, the algorithm initially samples actions from a unoptimized Normal distribution.  After many iterations, the Actor has augmented and refined it's sampling distribution for action selection through input from the Critic network.  The Critic network learns to effectively evaluate each action result and provide meaningful information for the Actor to optimize its sampling distribution for better gameplay!
